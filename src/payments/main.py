...
from .commands import CommandPayTransaction, CommandReversePayment, PayTransactionPayload, ReversePaymentPayload
...
async def app_startup():
    global tasks
    task1 = asyncio.ensure_future(subscribe_to_topic("payment-events", "sub-payments", EventPayment))
    task2 = asyncio.ensure_future(subscribe_to_topic("command-pay-transaction", "sub-com-pay-transaction", CommandPayTransaction))
    task3 = asyncio.ensure_future(subscribe_to_topic("command-reverse-payments", "sub-com-reverse-payments", CommandReversePayment))
    tasks.append(task1)
    tasks.append(task2)
    tasks.append(task3)
