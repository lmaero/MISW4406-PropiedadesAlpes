...
from pda.modules.tenant.infrastructure.repositories import TenantsRepository
...
class TenantService(Service):
    ...
    def create_tenant(self, tenant_dto: TenantDTO) -> TenantDTO:
        tenant: Tenant = self.tenant_factory.create_object(tenant_dto, TenantMapper())
        tenant.create_tenant(tenant)

        repository = self.repository_factory.create_object(TenantsRepository.__class__)
