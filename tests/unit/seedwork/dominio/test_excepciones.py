...
from pda.seedwork.domain.exceptions import (
    DomainException,
    ImmutableEntityIdException,
...
class ExcepcionPrueba(DomainException): ...
...
def test_IdDebeSerInmutableExcepcion_es_creable_y_ejecutable():
    with pytest.raises(DomainException):
        ...
        excepcion = ImmutableEntityIdException(message=mensaje)
...
def test_ReglaNegocioExcepcion_es_creable_y_ejecutable():
    with pytest.raises(DomainException):
        ...
        excepcion = BusinessRuleException(regla_negocio)
